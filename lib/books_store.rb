...
class BooksStore
  ...
  # Новые методы
  def first_letter_by_surnames
    letter_surnames = []
    @authors.each_value do |author|
      letter_surnames.append(author.letter)
    end
    letter_surnames.uniq.sort.append('Завершить работу приложения')
  end

  def info_author_by_letter(letter)
    @authors.values.select do |author|
  ...
  # где автор хотя бы есть
  def count_books_author(id)
    books_author = 0
    @books.each_value do |book|
      books_author += book.relationships['authors'].select do |author|
        author['id'] == id
      end.size
    end
    books_author
  ...
  def count_books_coauthored(id)
    books_author = 0
    @books.each_value do |book|
      next unless book.relationships['authors'].size > 1
  ...
  def coauthoreds(id)
    ...
    @books.each_value do |book|
      ...
      book.relationships['authors'].each do |author|
        next unless author['id'] == id

        book.relationships['authors'].each do |hash_author|
          list_id_coauthoreds += [hash_author['id']]
        end
      end
    end

    list_id_coauthoreds.uniq!
    list_id_coauthoreds.delete(id)
    list_id_coauthoreds.each do |id_coauthored|
      coauthoreds += "#{@authors[id_coauthored].name}\n"
    end
    coauthoreds
  end

  # Все книги автора отсортед
  def all_books_author(id)
    books = []
    @books.each_value do |book|
      book.relationships['authors'].each do |author|
        books.append(book.attributes['name']) if author['id'] == id
      end
    end
    books.sort!
    books_str = ''
